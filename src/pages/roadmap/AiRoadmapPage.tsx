...
import { ArrowLeft, Search, Grid3x3, List, Filter, Info } from "lucide-react";
...
// V3 Phase 3: Roadmap creation only allowed through journey flow
...
const AiRoadmapPage = () => {
  // V3 Phase 3: Always in list view, creation through journey flow only
  const [learningRoadmapSource, setLearningRoadmapSource] = useState<
    RoadmapSessionSummary[]
  >([]);
  const [deletedRoadmapSource, setDeletedRoadmapSource] = useState<
    RoadmapSessionSummary[]
  >([]);
  const [statusCounts, setStatusCounts] = useState<RoadmapStatusCounts>(
    INITIAL_STATUS_COUNTS,
  );
  const [isLoading, setIsLoading] = useState(false);
  const [isLoadingList, setIsLoadingList] = useState(true);
  const [isLoadingDeletedList, setIsLoadingDeletedList] = useState(false);
  const [activeListScope, setActiveListScope] =
    useState<RoadmapListScope>("learning");
  const [hasLoadedDeletedRoadmaps, setHasLoadedDeletedRoadmaps] =
    useState(false);
  const { toast, isVisible, showError, showSuccess, showToast, hideToast } =
    useToast();
  ...
  const buildFallbackStatusCounts = useCallback(
    (
      ...
      const active = learningList.filter(
        (item) => normalizeRoadmapStatus(item.status) === "ACTIVE",
      ).length;
      const paused = learningList.filter(
        (item) => normalizeRoadmapStatus(item.status) === "PAUSED",
      ).length;
  ...
  const loadLearningRoadmaps = useCallback(async () => {
    ...
    try {
      ...
      const deletedHint = hasLoadedDeletedRoadmaps
  ...
  const loadDeletedRoadmaps = useCallback(
    async (force = false) => {
      if (!isAuthenticated) {
        return;
      }
      ...
      try {
        ...
        await refreshStatusCounts(
          learningRoadmapSourceRef.current,
          normalizedDeletedRoadmaps.length,
        );
      } catch (error) {
        showError("Lỗi", (error as Error).message);
      } finally {
    ...
    [hasLoadedDeletedRoadmaps, isAuthenticated, refreshStatusCounts, showError],
  );
  ...
  // V3 Phase 3: Redirect to journey page for creating new roadmap
  // Roadmap creation is only allowed through journey flow
  const handleCreateRoadmap = () => {
    navigate("/journey");
  };

  // V3 Phase 3: Roadmap generation disabled - must go through journey flow
  // Roadmaps are automatically created when user completes journey assessment
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  const handleGenerate = async (_request: unknown) => {
    showError(
      "Tạo lộ trình qua Journey",
      "Vui lòng truy cập trang Hành trình (Journey) để tạo lộ trình học tập mới. Lộ trình sẽ được tự động tạo sau khi bạn hoàn thành bài đánh giá.",
      8,
    );
    navigate("/journey");
  };
  ...
  const handleNavigateRoadmap = (
    ...
    if (status === "PAUSED") {
      showError(
        "Roadmap đang tạm dừng",
        "Hãy kích hoạt lại roadmap để tiếp tục học.",
      );
      return false;
    }
    return true;
   ...
   */
  const runRoadmapAction = useCallback(
    async (
      sessionId: number,
      action: () => Promise<void>,
      successTitle: string,
      ...
      try {
        ...
        showError(
          "Không thể cập nhật trạng thái roadmap",
  ...
  const handleActivateRoadmap = useCallback(
    async (sessionId: number) => {
      const roadmap = learningRoadmapSourceRef.current.find(
  ...
  const handleDeleteRoadmap = useCallback(
    (sessionId: number) => {
      ...
      showToast({
        ...
        title: "Xóa roadmap?",
        message:
          "Roadmap sẽ chuyển sang danh sách đã xóa. Bạn vẫn có thể xóa vĩnh viễn ở danh sách đã xóa.",
        autoCloseDelay: 10,
        showCountdown: false,
        actionButton: {
          text: "Xóa",
          onClick: async () => {
            ...
            const oldStatus = normalizeRoadmapStatus(roadmap.status);
            setLearningRoadmapSource((current) =>
              current.filter((r) => r.sessionId !== sessionId),
            );
            setStatusCounts((prev) => ({
              ...prev,
              active:
  ...
  const handlePermanentDeleteRoadmap = useCallback(
    (sessionId: number) => {
      if (actionLoadingId !== null) {
        return;
      }

      const roadmap = deletedRoadmapSourceRef.current.find(
        (r) => r.sessionId === sessionId,
      );
      if (!roadmap) return;
      ...
      showToast({
        ...
        message:
          "Hành động này không thể hoàn tác. Toàn bộ dữ liệu gắn trực tiếp với roadmap sẽ bị xóa.",
        autoCloseDelay: 12,
        showCountdown: false,
        actionButton: {
          text: "Xóa vĩnh viễn",
          onClick: async () => {
            ...
            try {
              ...
              showSuccess(
                ...
                "Roadmap và dữ liệu liên quan đã được xóa khỏi hệ thống.",
                4,
              );
              if (hasLoadedDeletedRoadmaps) {
                await loadDeletedRoadmaps(true);
              ...
              showError(
                "Không thể xóa vĩnh viễn roadmap",
                (error as Error).message,
              );
            } finally {
              setActionLoadingId(null);
  ...
  const handleRestoreRoadmap = useCallback(
    async (sessionId: number) => {
      ...
      try {
        ...
        showSuccess(
          "Đã khôi phục roadmap",
          "Roadmap đã được khôi phục về danh sách còn sử dụng. Bạn có thể kích hoạt lại bất kỳ lúc nào.",
          4,
        );
        await loadLearningRoadmaps();
        if (hasLoadedDeletedRoadmaps) {
  ...
  const handleSwitchListScope = useCallback(
    (scope: RoadmapListScope) => {
  ...
  return (
    <div className="roadmap-hud-container">
      ...
      <div
        className="roadmap-page__container"
      ...
      >
        ...
        <div className="roadmap-page__header" style={{ marginBottom: "30px" }}>
          ...
          <div className="roadmap-page__header-content">
            <h1
              className="roadmap-page__title"
              style={{
                fontFamily: "var(--hud-font-mono)",
                color: "var(--hud-accent)",
                textShadow: "0 0 10px var(--hud-accent-glow)",
              }}
            >
              QUẢN LÝ LỘ TRÌNH
            </h1>
            <p
              className="roadmap-page__subtitle"
              style={{ color: "var(--hud-text-dim)" }}
            >
              Hệ thống lộ trình học tập AI
            </p>
          </div>
        </div>

        {/* V3 Phase 3: Info Banner - Journey Flow Only */}
        <div
          className="sv-roadmap-info-banner"
          style={{
          ...
          <div className="roadmap-page__list">
            ...
            {isAuthenticated && (
              ...
              >
                ...
                <div className="sv-roadmap-controls__filters">
                  ...
                  <div className="sv-roadmap-controls__group">
                    <span
                      className="sv-roadmap-controls__label"
                      style={{ color: "var(--hud-text-dim)" }}
                    >
                      Sắp xếp:
                    </span>
            ...
            {!isAuthenticated || isLoadingList ? (
              ...
              <div className="sv-roadmap-split-sections">
                <div
                  className="sv-roadmap-switch-tabs"
                  role="tablist"
                  aria-label="Chuyển danh sách roadmap"
                >
                  <button
                    type="button"
                    role="tab"
                  ...
                  >
                    ...
                    <span className="sv-roadmap-switch-tab__count">
                      {learningRoadmapCount}
                    </span>
                  </button>
                  <button
                    type="button"
                ...
                {activeListScope === "learning" ? (
                  <section className="sv-roadmap-split-section">
                    <div className="sv-roadmap-split-section__header">
                      <div>
                        <h2 className="sv-roadmap-split-section__title">
                          Roadmap còn sử dụng
                        </h2>
                        <p className="sv-roadmap-split-section__subtitle">
                          Danh sách roadmap ACTIVE và PAUSED để bạn tiếp tục
                          học.
                        </p>
                      </div>
                      <span className="sv-roadmap-split-section__count">
                        {learningRoadmapCount}
                      </span>
                    </div>
                    <RoadmapList
                  ...
                  <section className="sv-roadmap-split-section sv-roadmap-split-section--archive">
                    <div className="sv-roadmap-split-section__header">
                      <div>
                        ...
                        <p className="sv-roadmap-split-section__subtitle">
                          Danh sách roadmap đã xóa, bạn có thể xóa vĩnh viễn tại
                          đây.
                        </p>
                      </div>
                      <span className="sv-roadmap-split-section__count sv-roadmap-split-section__count--archive">
                        {deletedRoadmapCount}
                      </span>
                    </div>
                    <RoadmapList
