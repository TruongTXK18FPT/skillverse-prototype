...
const logCourseDraftOrderDebug = (
  label: string,
...
const normalizeRevisionSnapshotPayload = (
        ...
        lessons: Array.isArray(module.lessons)
          ? module.lessons.map((lesson, lessonIndex) => {
              ...
              return {
                ...
                ...(hasOwnKey(lesson, "assignmentCriteria")
                  ? {
                      assignmentCriteria: Array.isArray(
...
const validateRevisionSnapshotPayload = (
  snapshot: RevisionSnapshotPayload,
): string | null => {
...
const mapCourseModulesToDraftModules = (
...
const mergeSnapshotModulesWithFallback = (
  ...
  const merged: ModuleDraft[] = snapshotModules.map((snapshotModule) => {
    ...
    const fallbackByLessonId = new Map<
      number,
      { lesson: LessonDraft; index: number }
    >();
    const fallbackByTypeAndOrder = new Map<
      string,
      { lesson: LessonDraft; index: number }
    >();
          ...
          lessons.map((lessonRaw, lessonIndex) => {
            ...
            const assignmentCriteria: AssignmentCriteriaDraft[] =
              criteriaRaw.map((criteriaRawItem, criteriaIndex) => {
                ...
                const maxPoints =
                  toPositiveNumberOrUndefined(criteriaRawItem.maxPoints) ?? 0;
                return {
...
const findMatchingModule = (
  ...
  return baselineModules.find(
    (candidate) => normalizeKey(candidate.title) === normalizeKey(module.title),
  );
};
...
const CourseCreationPage = () => {
    ...
    const loadRevision = async () => {
      try {
        ...
        const fallbackModules = mapCourseModulesToDraftModules(
          state.modules as Array<Record<string, unknown>>,
        );
      ...
      try {
        ...
        if (isRevisionMode && activeRevision) {
          ...
          logCourseDraftOrderDebug(
            ...
            normalizedSnapshotPayload.modules as ModuleDraft[],
          );
            ...
            {(() => {
              ...
              return (
                <div
                  ...
                  onClick={() =>
                    setActiveView({ type: "module", moduleId: module.id })
                  }
                >
                  <div className="cb-sidebar__item-label">
                    <FiList />
                    ...
                    >
                      <div className="cb-sidebar__item-label">
                        ...
                        <span className="cb-sidebar__lesson-text">
                          <span>
                            {lIndex + 1}. {lesson.title || "(Chưa có tiêu đề)"}
                          </span>
                          <span
                            className="cb-sidebar__lesson-badge"
                            style={{
  ...
  const renderMainContent = () => {
                    ...
                    <div className="cb-info-hero-grid__left">
                      <div className="cb-form-group">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            updateCourseForm({ title: e.target.value })
                          }
                        />
                      </div>
                      ...
                      <div className="cb-form-group">
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateCourseForm({ summary: e.target.value })
                          }
                    ...
                    <div className="cb-info-subsection">
                      <div className="cb-form-group">
                        ...
                        <textarea
                          ...
                          onChange={(e) =>
                            updateCourseForm({ description: e.target.value })
                          }
                        />
                      </div>
                    </div>
                  ...
                  <div className="cb-grid cb-grid--2">
                    <div className="cb-form-group">
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          updateCourseForm({ category: e.target.value })
                        }
                      >
    ...
    if (activeView.type === "module") {
      ...
      if (!module)
        return <div className="cb-empty-state">Module không tồn tại</div>;
          ...
          <div className="cb-panel">
            <div className="cb-panel__header">
              <div className="cb-panel__title">
                Chỉnh sửa Module
                {isRevisionMode &&
                  (isNewModule || moduleChangedFields.length > 0) && (
                    <span
                      className="cb-sidebar__lesson-badge"
                      style={{
                        marginLeft: 10,
                      ...
                      title={
                        moduleChangedFields.length > 0
                          ? `Đã thay đổi: ${moduleChangedFields.join(", ")}`
                          : "Module mới tạo"
                      }
                    >
                          ...
                          (att: LessonAttachmentDraft, idx: number) => (
                            ...
                            >
                              ...
                              <button
                                className="cb-icon-button"
                                onClick={() => {
                                  const newAtts = lesson.attachments?.filter(
                                    (_, i) => i !== idx,
                                  );
                                  updateLessonField(module.id, lesson.id, {
                                    attachments: newAtts,
              ...
              {lesson.type === "video" && (
                <div className="cb-form-group">
                  ...
                  <div className="cb-tabs cb-video-source-tabs">
                    <button
                      className={`cb-tab cb-video-source-tab ${!lesson.videoMediaId && lesson.videoMediaId !== 0 ? "cb-tab--active" : ""}`}
                      onClick={() =>
                        updateLessonField(module.id, lesson.id, {
                          videoMediaId: undefined,
                        })
              ...
              {lesson.type === "assignment" && (
                <div className="cb-form-group">
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      updateLessonField(module.id, lesson.id, {
                        assignmentSubmissionType: e.target
                    ...
                    <div className="cb-criteria-list">
                      {(lesson.assignmentCriteria || []).map(
                        (crit: AssignmentCriteriaDraft, idx: number) => {
                          const criteriaError =
                            lessonErrors?.criteriaItems?.[idx];
                          return (
                            <div
                              key={idx}
                            ...
                            >
                              ...
                              <button
                                ...
                                onClick={() => {
                                  const newCrit =
                                    lesson.assignmentCriteria?.filter(
                                      (_, i) => i !== idx,
                                    );
                                  updateLessonField(module.id, lesson.id, {
                                    assignmentCriteria: newCrit,
                      ...
                      <div className="cb-ai-grading__title-wrap">
                        ...
                        <div>
                          ...
                          <p className="cb-ai-grading__subtitle">
                            Tự động gợi ý điểm dựa trên rubric và phong cách
                            chấm.
                          </p>
                        </div>
                      </div>
                      ...
                      <div className="cb-ai-grading__body">
                        ...
                        {(!lesson.assignmentCriteria ||
                          ...
                          <div className="cb-ai-grading__warning">
                            <FiAlertTriangle />
                            <span>
                              Bật AI chấm bài cần có rubric (tiêu chí chấm
                              điểm). Vui lòng thêm tiêu chí ở trên.
                            </span>
                          </div>
                        )}
                        ...
                        <div className="cb-ai-grading__note">
                          <FiInfo />
                          <span>
                            Khi bật AI chấm bài: chỉ chấp nhận file{" "}
                            <strong>PDF</strong> và <strong>DOCX</strong>, tối
                            đa 10MB. Mentor luôn xác nhận kết quả trước khi
                            student nhận điểm.
                          </span>
                        </div>
                      </div>
              ...
              {lesson.type === "quiz" && (
                <div className="cb-quiz-editor">
                  ...
                  {lessonEditor.activeTab === "settings" ? (
                    ...
                    <div
                      className="cb-questions-manager"
                      style={{ marginTop: 16 }}
                    >
                      {(lesson.questions || []).map(
                        (q: QuizQuestionDraft, idx: number) => (
                          <div key={q.id} className="cb-question-card">
                            ...
                            >
                              ...
                              {(q.type === QuestionType.MULTIPLE_CHOICE ||
                                ...
                                <>
                                  {q.options.map(
                                    (opt: QuizOptionDraft, oIdx: number) => (
                                      <div
                                        key={opt.id}
                                        className="cb-option-row"
                                      >
                                        <input
                                          className="cb-input cb-input--compact"
                                          value={opt.text}
                  ...
                  >
                    <div className="cb-revision-row__info">
                      <div className="cb-revision-row__line">
                        <span
                          className={`cb-revision-status cb-revision-status--${getRevisionStatusTone(revision.status)}`}
                        >
                      ...
                      <div className="cb-revision-row__meta">
                        <span>
                          Tạo: {formatRevisionDate(revision.createdAt)}
                        </span>
                        <span>
                          Cập nhật: {formatRevisionDate(revision.updatedAt)}
                        </span>
                      </div>
                    </div>
                    <button
        ...
        >
          ...
          >
            ...
            <div className="cb-form-group">
              ...
              <input
                className="cb-input cb-reading-link-modal__input"
