...
const getAutoReleaseCountdown = (
  ...
  return { text: `Còn ${remainingHours}h chờ xác nhận`, level: "normal" };
};
...
const BookingDetailPage: React.FC = () => {
  ...
  const counterpartAvatar =
    viewerRole === "mentor" ? booking?.learnerAvatar : null;
  ...
  const meetingVisible =
    ...
    (booking.status === "ONGOING" ||
      (booking.status === "CONFIRMED" &&
        !!startDate &&
        startDate.getTime() - now.getTime() <= 30 * 60 * 1000));
  const canOpenDispute =
    ...
    (booking.status === "PENDING_COMPLETION" ||
      ((booking.status === "CONFIRMED" || booking.status === "ONGOING") &&
        sessionEnded));
  const canReview =
    viewerRole === "learner" && booking?.status === "COMPLETED" && !review;
  const isPaymentSettled =
    !!booking?.paymentReference ||
    (!!booking &&
      PAID_BOOKING_STATUSES.includes(
        booking.status as (typeof PAID_BOOKING_STATUSES)[number],
      ));
  const parsedReview = useMemo(
    () => parseReviewPayload(review?.comment),
    [review?.comment],
  );
  const isFileEvidenceType = ["FILE", "IMAGE", "SCREENSHOT"].includes(
    evidenceType,
  );
  const isTextEvidenceType = ["TEXT", "LINK", "CHAT_LOG"].includes(
    evidenceType,
  );
  const hasEvidenceDraft = isFileEvidenceType
    ? !!evidenceFile
    : evidenceContent.trim().length > 0 ||
  ...
  const canSubmitOpenDispute =
    !!disputeReason.trim() &&
    (!hasEvidenceDraft ||
      (isFileEvidenceType ? !!evidenceFile : !!evidenceContent.trim()));
  const shouldShowEvidenceInputForComposer = isTextEvidenceType;
  const shouldShowFileInputForComposer = isFileEvidenceType;

  const money = useMemo(() => {
    if (!booking)
      return { userSpend: 0, refund: 0, mentorPay: 0, adminFee: 0, escrow: 0 };
  ...
  const handleStartMeeting = async () => {
    ...
    setBusy(true);
    try {
      const updated = booking.meetingLink
        ? booking
  ...
  const handleOpenDispute = async () => {
    ...
    setBusy(true);
    try {
      const newDispute = await openBookingDispute(
        booking.id,
        disputeReason.trim(),
      );
      ...
      showAppSuccess(
        "Đã mở tranh chấp",
        hasEvidenceDraft
          ? "Đã mở tranh chấp và gửi kèm bằng chứng."
          : "Bạn có thể bổ sung bằng chứng ngay trong lúc này.",
      );
      await loadData();
    } catch (err: any) {
      showAppError(
  ...
  const handleSubmitEvidence = async () => {
    ...
    try {
      ...
      showAppSuccess(
        "Đã gửi bằng chứng",
        "Thông tin tranh chấp đã được cập nhật.",
      );
    } catch (err: any) {
      showAppError(
        "Không thể gửi bằng chứng",
        err?.response?.data?.message || err?.message || "Vui lòng thử lại.",
      );
    } finally {
      setBusy(false);
  ...
  const handleReply = async (evidenceId: number) => {
    ...
    try {
      await respondToBookingDisputeEvidence(
        dispute.id,
        evidenceId,
        responseMap[evidenceId].trim(),
      );
      setEvidence(await getBookingDisputeEvidence(dispute.id));
  ...
  const handleSubmitReview = async () => {
    ...
    if (reviewRating === 0)
      errors.rating = "Vui lòng chọn số sao đánh giá (1–5 sao).";
  ...
  const learnerCompleted = !!(
    booking.learnerCompletedAt || booking.confirmedByLearner
  );
  const status =
    viewerRole === "learner" &&
    booking.status === "PENDING_COMPLETION" &&
    ...
    <div className="bkd-shell">
      ...
      <div className="bkd-topbar">
        <button
          className="bkd-back"
          onClick={() =>
            navigate(viewerRole === "mentor" ? "/mentor" : "/my-bookings")
          }
        >
          <ArrowLeft size={16} />
          Quay lại
        ...
        <div className="bkd-hero-right">
          <div className="bkd-price-neon">
            ...
            <div>
              ...
              <strong className="bkd-price-value">
      ...
      <section className="bkd-action-bar">
        <div className="bkd-action-bar__left">
          {viewerRole === "mentor" && booking.status === "PENDING" && (
            <>
              <button
                className="bkd-btn bkd-btn-approve"
                onClick={() =>
                  runAction(
              ...
              <button
                className="bkd-btn bkd-btn-danger"
                onClick={async () => {
                  const reason = window
                    .prompt("Lý do từ chối booking", "")
                    ?.trim();
                  if (reason)
                    await runAction(
          ...
          {meetingVisible && (
            <button
              className="bkd-btn bkd-btn-meeting"
              onClick={handleStartMeeting}
              disabled={busy}
            >
              <Video size={16} />{" "}
              {booking.meetingLink ? "Vào phòng học" : "Bắt đầu"}
            </button>
          )}
          {viewerRole === "mentor" &&
            sessionEnded &&
            ["CONFIRMED", "ONGOING"].includes(booking.status) && (
              <button
                className="bkd-btn bkd-btn-complete"
                onClick={() =>
                  runAction(
                    () => completeBooking(booking.id),
                    "Đã đánh dấu hoàn tất",
                    "Hệ thống đang chờ learner xác nhận.",
                  )
                }
                disabled={busy}
              >
              ...
              <button
                ...
                onClick={() =>
                  runAction(
                    () => completeBooking(booking.id),
                    "Đã xác nhận",
                    "Cả hai bên đã đồng ý. Buổi học hoàn tất.",
                  )
                }
                disabled={busy}
              >
          ...
          {viewerRole === "learner" &&
            ["PENDING", "CONFIRMED"].includes(booking.status) && (
              <button
                ...
                onClick={async () => {
                  if (
                    await confirmAction("Bạn chắc chắn muốn hủy booking này?")
                  )
                    await runAction(
              ...
              <button
                ...
                onClick={async () => {
                  if (await confirmAction("Xác nhận hoàn tất buổi học?"))
                    await runAction(
                      () => confirmCompleteBooking(booking.id),
                      "Đã hoàn tất",
          ...
          {viewerRole === "learner" &&
            booking.status === "PENDING_COMPLETION" &&
            learnerCompleted && (
              <span className="bkd-status-pill bkd-tone-purple">
                Chờ mentor xác nhận
              </span>
            )}
        </div>
        <div className="bkd-action-bar__right">
          ...
          {(booking.paymentReference ||
            ["COMPLETED", "PENDING_COMPLETION"].includes(booking.status)) && (
            <button
              className="bkd-btn bkd-btn-secondary"
              onClick={async () => {
                setBusy(true);
          ...
          {booking.meetingLink && (
            <button
              ...
              onClick={async () => {
                ...
                showAppSuccess(
                  "Đã sao chép",
                  "Meeting link đã được copy vào clipboard.",
                );
              }}
            >
        ...
        <div className="bkd-card">
          ...
          <div className="bkd-info-list">
            ...
            <div className="bkd-info-row">
              <span className="bkd-info-label">Loại booking</span>
              {(() => {
                const typeConfig =
                ...
                return (
                  <span
                    className={`bkd-type-badge ${isRoadmapMentoring(booking) ? "bkd-type-badge--roadmap" : ""}`}
                    style={
                      {
            ...
            <div className="bkd-info-row">
              ...
              <span className="bkd-info-value">
                {safeFormatDate(booking.startTime, {
                  weekday: "long",
                  year: "numeric",
                  month: "long",
                  day: "numeric",
            ...
            <div className="bkd-info-row">
              ...
              <span
                className={`bkd-info-value ${isRoadmapMentoring(booking) ? "bkd-info-value--cyan" : ""}`}
              >
                {isRoadmapMentoring(booking)
            ...
            {(() => {
              ...
              return (
                <div className="bkd-info-row">
                  <span className="bkd-info-label">Tự động hoàn tiền</span>
                  <span
                    className={`bkd-auto-release bkd-auto-release--${autoRelease.level}`}
                  >
                    <Clock size={12} />
                    {autoRelease.text}
                  </span>
            ...
            <div className="bkd-info-row">
              ...
              <span
                className={`bkd-info-value bkd-info-value--${isPaymentSettled ? "green" : "amber"}`}
              >
            ...
            {!isRoadmapMentoring(booking) && (
              <div className="bkd-info-row">
                ...
                <span
                  className={`bkd-info-value bkd-info-value--${booking.meetingLink ? "cyan" : "slate"}`}
                >
        ...
        <div className="bkd-card">
          ...
          <div className="bkd-finance-list">
            <div className="bkd-finance-row bkd-finance-row--highlight">
              ...
              <strong className="bkd-finance-value bkd-finance-value--cyan">
                {formatCurrency(booking.priceVnd)}
              </strong>
            </div>
            {viewerRole === "learner" ? (
              <>
                <div className="bkd-finance-row">
                  ...
                  <span className="bkd-finance-value">
                    {formatCurrency(money.userSpend)}
                  </span>
                </div>
                <div className="bkd-finance-row">
                  <span className="bkd-finance-label">Hoàn tiền</span>
                  <span
                    className={`bkd-finance-value ${money.refund > 0 ? "bkd-finance-value--orange" : ""}`}
                  >
                    {formatCurrency(money.refund)}
                  </span>
                </div>
              </>
            ) : (
              <>
                ...
                <div className="bkd-finance-row">
                  ...
                  <span className="bkd-finance-value bkd-finance-value--green">
                    {formatCurrency(money.mentorPay)}
                  </span>
                </div>
                <div className="bkd-finance-row">
                  <span className="bkd-finance-label">Phí hệ thống (20%)</span>
                  <span className="bkd-finance-value bkd-finance-value--slate">
                    {formatCurrency(money.adminFee)}
                  </span>
                </div>
              </>
            )}
            {money.escrow > 0 && (
              <div className="bkd-finance-row">
                ...
                <span className="bkd-finance-value bkd-finance-value--amber">
                  {formatCurrency(money.escrow)}
                </span>
              </div>
            )}
          </div>
              ...
              <div className="bkd-review-display-stars">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star
                    ...
                    strokeWidth={1.5}
                    className={
                      star <= review.rating ? "bkd-star-glow" : "bkd-star-dim"
                    }
                  />
                ))}
            ...
            {(parsedReview.tags.length > 0 || parsedReview.message) && (
              <div className="bkd-review-display-body">
                ...
                {parsedReview.message && (
                  <p className="bkd-review-display-comment">
                    {parsedReview.message}
                  </p>
                )}
              </div>
            ...
            >
              ...
              <div className="bkd-review-step">
                <div className="bkd-review-step-header">
                  <div className="bkd-review-step-num bkd-review-step-num--1">
                    1
                  </div>
                  <span>Bạn hài lòng thế nào với buổi học?</span>
                </div>
                <div
                  className={`bkd-star-picker-wrap ${reviewErrors.rating ? "bkd-star-error" : ""}`}
                >
                  {[1, 2, 3, 4, 5].map((star) => (
                    <button
                      key={star}
                ...
                <div className="bkd-star-hint-large">
                  {reviewRating === 0 && (
                    <span className="bkd-hint-neutral">
                      Chọn số sao để đánh giá
                    </span>
                  )}
                  {reviewRating === 1 && (
                    <span className="bkd-hint-1">😞 Rất không hài lòng</span>
                  )}
                  {reviewRating === 2 && (
                    <span className="bkd-hint-2">😕 Không hài lòng</span>
                  )}
                  {reviewRating === 3 && (
                    <span className="bkd-hint-3">😐 Bình thường</span>
                  )}
                  {reviewRating === 4 && (
                    <span className="bkd-hint-4">🙂 Hài lòng</span>
                  )}
                  {reviewRating === 5 && (
                    <span className="bkd-hint-5">
              ...
              <div className="bkd-review-step bkd-review-step--tags">
                <div className="bkd-review-step-header">
                  <div className="bkd-review-step-num bkd-review-step-num--2">
                    2
                  </div>
                ...
                <div className="bkd-tag-grid-large">
                  {REVIEW_TAGS.map((tag) => {
                    ...
                    return (
                      ...
                      >
                        <div className="bkd-tag-icon">
                          <Icon size={20} />
                        </div>
                        <span>{tag.label}</span>
                        {selected && (
                          <CheckCircle2 size={14} className="bkd-tag-check" />
                        )}
                      </button>
                    );
                  })}
              ...
              <div className="bkd-review-step bkd-review-step--comment">
                <div className="bkd-review-step-header">
                  <div className="bkd-review-step-num bkd-review-step-num--3">
                    3
                  </div>
                ...
                <div className="bkd-char-count-row">
                  <span className="bkd-char-hint">
                    Tối thiểu 10 ký tự để gửi
                  </span>
                  <span
                    className={`bkd-char-count ${reviewComment.length < 10 ? "bkd-char-low" : "bkd-char-ok"}`}
                  >
      ...
      <section className="bkd-card bkd-dispute-section">
        <div className="bkd-card-header">
          <AlertTriangle
            size={18}
            className="bkd-card-header-icon bkd-card-header-icon--orange"
          />
          <h2>Tranh chấp & Bằng chứng</h2>
          {canOpenDispute && !openComposer && (
            <button
              className="bkd-btn bkd-btn-dispute"
          ...
          {canOpenDispute && openComposer && (
            ...
            >
              <div className="bkd-dispute-composer-head">
                <h3>Mở tranh chấp</h3>
                <p>
                  Nêu rõ vấn đề và có thể gửi kèm bằng chứng ngay trong lần gửi
                  đầu tiên.
                </p>
              </div>
                  ...
                  <div className="bkd-dispute-field">
                    ...
                    {!evidenceFile ? (
                      <>
                        <button
                          type="button"
                          className="bkd-upload bkd-upload--full"
                          onClick={() => composerFileInputRef.current?.click()}
                        >
                          <FileText size={15} /> Chọn tệp đính kèm
                        </button>
                        <p className="bkd-upload-hint">
                          Hỗ trợ ảnh, PDF, DOC, DOCX, TXT. Tối đa 10MB.
                        </p>
                      </>
                    ) : (
                      <div className="bkd-file-preview">
                        ...
                        <div className="bkd-file-preview__info">
                          <span className="bkd-file-preview__name">
                            {evidenceFile.name}
                          </span>
                          <span className="bkd-file-preview__size">
                            {formatFileSize(evidenceFile.size)}
                          </span>
                        </div>
                        <button
                          type="button"
                          className="bkd-file-preview__remove"
                          onClick={() => handleEvidenceFileSelected(null)}
                          title="Bỏ chọn"
                        >
                          <XCircle size={16} />
                        </button>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        handleEvidenceFileSelected(e.target.files?.[0] || null)
                      }
                    />
                  </div>
                )}
              ...
              <div className="bkd-dispute-composer-actions">
                ...
                <button
                  ...
                  disabled={busy || !canSubmitOpenDispute}
                >
                  {busy ? (
                    <>
                      <Loader2 size={15} className="bkd-spin" /> Đang gửi...
                    </>
                  ) : (
                    <>
        ...
        {dispute &&
          (() => {
            ...
            const resolutionMeta = dispute.resolution
              ? DISPUTE_RESOLUTION_META[dispute.resolution] || {
              ...
              <div className="bkd-dispute-banner">
                <div className="bkd-dispute-banner-left">
                  <div className="bkd-kicker">
                    Dispute #{dispute.id}
                    <span
                      className={`bkd-tone-${statusMeta.tone} bkd-dispute-status-pill`}
                    >
                      {statusMeta.label}
                    </span>
                  </div>
                  <p className="bkd-dispute-reason">
                    {dispute.reason || "Không có lý do bổ sung."}
                  </p>
                </div>
                {resolutionMeta && (
                  <div
                    className={`bkd-resolution-chip bkd-resolution-chip--${resolutionMeta.tone}`}
                  >
                    <strong>{resolutionMeta.label}</strong>
                    <span>
                      {safeFormatDate(dispute.resolvedAt) || "Đang xử lý"}
                    </span>
                  </div>
                )}
              </div>
                  ...
                  >
                    <header className="bkd-evidence-card__header">
                      ...
                      <div className="bkd-evidence-card__header-right">
                        <span className="bkd-evidence-time">
                    ...
                    {item.fileUrl && (
                      <div className="bkd-evidence-file-wrap">
                        {isImageFile(item.fileName || item.fileUrl) ? (
                          <a
                            className="bkd-evidence-image-link"
                            href={item.fileUrl}
                            target="_blank"
                            rel="noreferrer"
                          >
                          ...
                          <a
                            className="bkd-file-link"
                            href={item.fileUrl}
                            target="_blank"
                            rel="noreferrer"
                          >
                            <FileText size={14} />{" "}
                            {item.fileName || "Xem tệp đã gửi"}
                          </a>
                        )}
                      </div>
                    )}
                    {item.description && (
                      <p className="bkd-evidence-note">{item.description}</p>
                    )}
                        ...
                        {item.responses.map((response) => (
                          <div key={response.id} className="bkd-response-item">
                            <div className="bkd-response-header">
                              ...
                              <span>
                                {safeFormatDate(response.createdAt, {
                                  hour: "2-digit",
                                  minute: "2-digit",
                                  day: "2-digit",
                                  month: "2-digit",
                    ...
                    {dispute.status !== "RESOLVED" && !isOwn && (
                      <div className="bkd-response-composer">
                        ...
                        <button
                          ...
                          disabled={
                            replyingId === item.id ||
                            !responseMap[item.id]?.trim()
                          }
                        >
                          {replyingId === item.id ? (
                            <Loader2 size={16} className="bkd-spin" />
                          ) : (
                            <Send size={16} />
                          )}{" "}
                          Gửi
            ...
            {dispute.status !== "RESOLVED" && (
              <aside className="bkd-evidence-composer">
                ...
                <input
                  ...
                  value={evidenceDescription}
                  onChange={(e) => setEvidenceDescription(e.target.value)}
                  placeholder="Mô tả ngắn cho bằng chứng"
                />
                <button
                  className="bkd-btn bkd-btn-review"
