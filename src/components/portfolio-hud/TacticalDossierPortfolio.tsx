                    ...
                    <div style={{ marginBottom: "1.5rem" }}>
                      ...
                      <div className="dossier-module-tags">
                        {getSkills().map((skill: string, idx: number) => {
                          ...
                          const isVerified = vs && (vs.verifiedByMentorId || (vs as any).source === 'MANUAL');
                          const isSelfStudy = vs && !isVerified;
                          return (
                            <span
                              key={idx}
                              className={`dossier-module-tag ${isVerified ? 'verified-skill-tag' : isSelfStudy ? 'self-study-skill-tag' : 'unverified-skill-tag'}`}
                              onClick={vs ? () => {
                                if (profile?.userId) {
                              ...
                              style={{
                                ...
                                ...(vs
                                  ? {
                                      cursor: 'pointer',
                                      borderColor: isVerified ? 'rgba(16, 185, 129, 0.5)' : 'rgba(167, 139, 250, 0.5)',
                                      background: isVerified ? 'rgba(16, 185, 129, 0.05)' : 'rgba(167, 139, 250, 0.05)'
                                    }
                                  : { borderColor: 'rgba(148, 163, 184, 0.2)', background: 'transparent' })
                              }}
                              title={isVerified ? "Kỹ năng đã được mentor/admin xác thực" : isSelfStudy ? "Kỹ năng hoàn thành từ roadmap tự học (Click để xem lộ trình)" : "Kỹ năng chưa được xác thực"}
                            >
                              ...
                              {isVerified ? (
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: '4px', background: 'rgba(167, 139, 250, 0.15)', padding: '3px 8px', borderRadius: '4px' }}>
                                  <BookOpen size={12} style={{ color: '#a78bfa' }} />
                                  <span style={{ fontSize: '0.65rem', color: '#a78bfa', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.5px' }}>Tự học</span>
                                </div>
                              ) : (
