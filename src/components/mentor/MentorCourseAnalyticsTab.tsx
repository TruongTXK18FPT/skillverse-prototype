...
type DatePreset = '7days' | '30days' | '90days' | 'thisMonth' | 'all' | 'custom';
...
const MentorCourseAnalyticsTab: React.FC = () => {
  ...
  const [enrollmentMap, setEnrollmentMap] = useState<Record<number, { total: number; completed: number }>>({});
    ...
    try {
      ...
      await Promise.allSettled(
        allCourses.map(async (course) => {
          try {
            const stats = await getCourseEnrollments(course.id, user.id, 0, 1);
            enrollMap[course.id] = {
              total: stats.totalElements,
              completed: 0, // completed from per-course detail if needed
            };
          } catch {
            enrollMap[course.id] = { total: course.enrollmentCount ?? 0, completed: course.completedCount ?? 0 };
          }
        })
      );
  ...
  const handlePresetChange = (preset: DatePreset) => {
    ...
    const range = getPresetDates(preset as Exclude<DatePreset, 'custom' | 'all'>);
  ...
  const totalStudents = useMemo(() =>
    publicCourses.reduce((sum, c) => sum + (enrollmentMap[c.id]?.total ?? c.enrollmentCount ?? 0), 0),
  ...
  const avgCompletionRate = useMemo(() => {
    ...
    const totalRate = publicWithEnrollment.reduce((sum, c) => {
      const enrolled = enrollmentMap[c.id]?.total ?? c.enrollmentCount ?? 0;
      const completed = enrollmentMap[c.id]?.completed ?? c.completedCount ?? Math.round(enrolled * 0.3);
    ...
    <div className="mca-wrapper">
      ...
      <div className="mca-header">
        <div className="mca-header-left">
          ...
          <p className="mca-subtitle">Hiệu suất và thống kê khóa học của bạn — {periodLabel}</p>
        </div>
      ...
      <div className="mca-filter-bar">
        <div className="mca-preset-group">
          {(['7days', '30days', '90days', 'thisMonth', 'all', 'custom'] as DatePreset[]).map(preset => (
            <button
              key={preset}
      ...
      <div className="mca-metrics">
        <div className="mca-metric-card students">
          ...
          <div className="mca-metric-body">
            <div className="mca-metric-value">{formatNumber(totalStudents)}</div>
        ...
        <div className="mca-metric-card revenue">
          ...
          <div className="mca-metric-body">
            <div className="mca-metric-value">{formatCurrency(totalRevenue)}</div>
            <div className="mca-metric-label">Tổng Doanh Thu</div>
            <div className="mca-metric-detail">
              Ước tính từ enrollment
            </div>
          </div>
          <div className="mca-metric-glow" />
        </div>
        ...
        <div className="mca-metric-card completion">
          ...
          <div className="mca-metric-body">
            <div className="mca-metric-value">{formatPercent(avgCompletionRate)}</div>
      ...
      <div className="mca-charts-row">
          ...
          <div className="mca-chart-shell">
            {enrollmentTrend.length > 0 ? (
              <ResponsiveContainer width="100%" height={220}>
                <LineChart data={enrollmentTrend} margin={{ top: 8, right: 8, left: -20, bottom: 0 }}>
                  <defs>
                    <linearGradient id="mca-enrollment-gradient" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#22d3ee" stopOpacity={0.4} />
                      <stop offset="100%" stopColor="#22d3ee" stopOpacity={0.05} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid stroke="rgba(6, 182, 212, 0.12)" vertical={false} />
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number) => [formatNumber(value), 'Enrollments']}
                    labelFormatter={(label) => enrollmentTrend.find(r => r.label === label)?.fullLabel || label}
                  />
                  <Line
                    type="monotone"
                    dataKey="enrollments"
        ...
        <div className="mca-panel">
          ...
          <div className="mca-chart-shell">
            {topCourses.length > 0 ? (
              <ResponsiveContainer width="100%" height={220}>
                <BarChart data={topCourses} layout="vertical" margin={{ top: 4, right: 10, left: 0, bottom: 0 }}>
                  <defs>
                    <linearGradient id="mca-course-bar" x1="0" y1="0" x2="1" y2="0">
                      <stop offset="0%" stopColor="#22d3ee" stopOpacity={0.85} />
                      <stop offset="100%" stopColor="#3b82f6" stopOpacity={0.85} />
                    </linearGradient>
                  </defs>
                  <CartesianGrid stroke="rgba(6, 182, 212, 0.12)" horizontal={false} />
                  <XAxis type="number" stroke="#94a3b8" tickLine={false} axisLine={false} fontSize={11} />
                  <YAxis dataKey="title" type="category" stroke="#94a3b8" tickLine={false} axisLine={false}
                  ...
                  <Tooltip
                    ...
                    formatter={(value: number) => [formatNumber(value), 'Học viên']}
                  />
                  <Bar dataKey="enrollmentCount" fill="url(#mca-course-bar)" radius={[0, 6, 6, 0]} barSize={18} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
        ...
        <div className="mca-panel mca-panel--wide">
          <div className="mca-panel-header">
            <div>
              ...
              <h4 className="mca-panel-title">Doanh thu theo khóa học (top 5)</h4>
            </div>
            <BarChart3 size={18} className="mca-panel-icon" />
          </div>
          <div className="mca-chart-shell">
            {revenueByCourse.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={revenueByCourse} margin={{ top: 4, right: 20, left: -10, bottom: 0 }}>
                  <defs>
                    <linearGradient id="mca-revenue-bar" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor="#22d3ee" stopOpacity={0.85} />
                  ...
                  <YAxis stroke="#94a3b8" tickLine={false} axisLine={false}
                    tickFormatter={(v) => `${Math.round(v / 1000000)}M`} fontSize={11} />
                  <Tooltip
                    contentStyle={{
                      background: 'rgba(13, 22, 35, 0.95)',
                      border: '1px solid rgba(6, 182, 212, 0.4)',
                    ...
                    labelFormatter={(label) => revenueByCourse.find(r => r.label === label)?.fullLabel || label}
                  />
                  <Bar dataKey="revenue" fill="url(#mca-revenue-bar)" radius={[6, 6, 0, 0]} barSize={28} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
        ...
        <div className="mca-panel">
          ...
          <div className="mca-chart-shell">
            {completionByCourse.length > 0 ? (
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={completionByCourse} margin={{ top: 4, right: 10, left: -10, bottom: 0 }}>
                  <defs>
                    <linearGradient id="mca-completion-bar" x1="0" y1="0" x2="0" y2="1">
                  ...
                  />
                  <YAxis stroke="#94a3b8" tickLine={false} axisLine={false}
                    tickFormatter={(v) => `${v}%`} fontSize={11} domain={[0, 100]} />
                  <Tooltip
                    contentStyle={{
                      background: 'rgba(13, 22, 35, 0.95)',
                      border: '1px solid rgba(6, 182, 212, 0.4)',
                    ...
                    labelFormatter={(label) => completionByCourse.find(r => r.name === label)?.fullName || label}
                  />
                  <Bar dataKey="rate" fill="url(#mca-completion-bar)" radius={[6, 6, 0, 0]} barSize={28} />
                </BarChart>
              </ResponsiveContainer>
            ) : (
